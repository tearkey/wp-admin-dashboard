...
export function AdminMenu({ collapsed, onToggleCollapsed, pendingComments }: AdminMenuProps) {
  ...
  return (
    <nav
      ...
      className={cn(
        "fixed top-8 bottom-0 left-0 z-40 flex flex-col bg-wp-menu font-wp text-[13px] text-wp-menu-text transition-[width] duration-150",
        collapsed ? "w-[36px]" : "w-[36px] md:w-[160px]",
      )}
    >
          ...
          return (
            <li key={item.label} className="group relative">
              ...
              {item.submenu.length > 0 && (
                <ul
                  className={cn(
                    "absolute top-0 left-full z-50 hidden min-w-[160px] bg-wp-menu-hover py-1 shadow-lg group-hover:block",
                  )}
                >
      ...
      <button
        ...
        className="flex h-[34px] shrink-0 items-center gap-2 px-2 text-wp-menu-icon hover:text-wp-blue-hover"
        aria-label={collapsed ? "Expand menu" : "Collapse menu"}
      >
        <ChevronLeft size={18} className={cn("transition-transform", collapsed && "rotate-180")} />
        {!collapsed && <span className="hidden md:inline">Collapse menu</span>}
      </button>
    </nav>
  );
}
