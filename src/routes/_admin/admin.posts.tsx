...
import type { PostStatus, WpPost } from "@/data/cms-mock";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Posts — Techtrick CMS" },
      {
        ...
        content: "All posts, filterable by status, author and search.",
      },
  ...
  const columns: Column<WpPost>[] = [
    {
      ...
      render: (p) => (
        <>
          ...
          {p.status !== "publish" && (
            <span className="ml-1.5 font-semibold text-tt-text">
  ...
  return (
    <div className="px-3 pt-2 pb-10 sm:px-5">
      ...
      <div className="mt-1 mb-3 flex flex-wrap items-center gap-2">
        <h1 className="text-[23px] leading-[1.3] font-normal text-tt-text">Posts</h1>
        <button
          type="button"
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex w-full items-center gap-1 sm:ml-auto sm:w-auto">
                ...
                <input
                  ...
                  className="h-11 w-full rounded border border-tt-border bg-tt-surface px-2 text-[16px] outline-none focus:border-tt-blue sm:w-[220px] md:h-[30px] md:text-[13px]"
                />
              </div>
            </div>
            <TableControls
